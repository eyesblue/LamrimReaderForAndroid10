	    	...
	    	while(downloadIndex< SpeechData.name.length){
	    		...
	    		synchronized(threadPool){
	    			if(downloadIndex>= SpeechData.name.length){
						Log.d(getClass().getName(),"Thread"+tId+" Terminate, End of media index reached.");
	    				return ;
	    			}
				...
				Log.d(getClass().getName(),"Thread"+tId+" get download task index "+downloadIndex);
				...
				if(!mediaFile.exists()){
					...
					isDwSuccess=HttpDownloader.download(DownloadAllService.this, rs.getMediaFileAddress(downloadingIndex), mediaFile.getAbsolutePath(), null);
					if(!isDwSuccess)
						hasFailure=true;
				}
